...
def _build_composio_context_prompt(connected_apps: list[str]) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Verxio Connected Apps",
            "",
            "Verxio app connections are managed through Composio Tool Router.",
            "",
            "Connected apps:",
            app_list,
            "",
            "How Composio tools work in Verxio (read carefully — this is the only working path):",
            "- The only Hermes-callable Composio tools are the Tool Router meta-tools:",
            "  `mcp_composio_COMPOSIO_SEARCH_TOOLS`, `mcp_composio_COMPOSIO_GET_TOOL_SCHEMAS`, and `mcp_composio_COMPOSIO_MULTI_EXECUTE_TOOL`.",
            ...
            "2. Call `mcp_composio_COMPOSIO_SEARCH_TOOLS` for that toolkit (example queries: googlesheets, create spreadsheet, gmail).",
            "3. If needed, call `mcp_composio_COMPOSIO_GET_TOOL_SCHEMAS` for the chosen slug(s).",
            "4. ALWAYS execute with `mcp_composio_COMPOSIO_MULTI_EXECUTE_TOOL`. Put the slug and arguments inside `tools` as a native JSON array of objects (never a stringified JSON blob).",
            "5. Return the real result to the user (spreadsheet URL, message id, etc.). Do not fall back to Notepad/CSV/manual upload unless MULTI_EXECUTE itself fails.",
            "",
            "Google Sheets / Drive create example shape:",
            '- `mcp_composio_COMPOSIO_MULTI_EXECUTE_TOOL` with `tools`: `[{"tool_slug":"GOOGLESHEETS_CREATE_GOOGLE_SHEET1","arguments":{"title":"..."}}]` (field names must match the schema from GET_TOOL_SCHEMAS).',
            "",
            "Google Docs long-content rules:",
            "- Pass `COMPOSIO_MULTI_EXECUTE_TOOL.tools` as a native JSON array of objects, never as a stringified JSON blob.",
            "- For reports longer than ~1500 characters: write the full markdown under `/workspace/artifacts/` first, create the doc, then populate in small section chunks via MULTI_EXECUTE. One huge markdown payload often fails JSON validation.",
            "- Prefer `read_file` on each artifact chunk, then one MULTI_EXECUTE per chunk.",
            "- If create succeeds but content write fails, retry with smaller chunks; do not claim Composio is disconnected.",
            "",
