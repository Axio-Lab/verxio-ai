...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    if (!desktop) {
      failDesktopBoot('Verxio web bridge is unavailable.')
    ...
    // composer stayed disabled forever on "Starting Verxio...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // as an opaque "Could not connect to Verxio gateway". resolveGatewayWsUrl
