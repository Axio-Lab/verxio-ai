...
// backend now surfaces the full runtime model universe (every canonical
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      const probe = await validateProviderCredential(keyEnv, apiKeyDraft.trim())

      if (!probe.reachable) {
        setError(probe.message || `Could not reach ${selectedProviderRow?.name ?? 'provider'}.`)

        return
      }

      if (!probe.ok) {
        setError(probe.message || 'Invalid API key.')
      ...
      // runtime model curation). Best-effort, fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Verxio runs the flow for you.`}
          </p>
        )}
