...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  timezone: 'Used when Verxio needs local time context. Blank uses the system timezone.',
  agent: {
    imageInputMode: 'Controls how image attachments are sent to the model.',
    maxTurns: 'Upper bound for tool-calling turns before Verxio stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters Verxio can read from one file request.',
  approvals: {
    mode: 'How Verxio handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When Verxio updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
