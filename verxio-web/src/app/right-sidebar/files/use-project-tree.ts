...
export interface UseProjectTreeResult {
   ...
   *  session's recorded cwd is unreachable and we fell back to the browser-
...
// reappears after a checkout, folder-access revoked), keep retrying on a slow
// cadence so the tree self-heals instead of staying "UNREADABLE" forever.
...
/** Sessions record their launch cwd; stale server paths or revoked browser
 *  folder access arrive here as directories that can't be read. Rather than
...
async function loadRoot(cwd: string, { force = false }: { force?: boolean } = {}) {
  ...
  if (error) {
    const fallback = fallbackRootFor(cwd)
...
export function useProjectTree(cwd: string): UseProjectTreeResult {
  ...
  // (folder access restored, worktree re-created) and switch back when it
