...
const STORAGE_KEY = 'verxio.web.model-presets'
 ...
 *  fall back to the default (medium effort, no fast). */
...
export async function applyModelPreset(
  ...
  const { request, sessionId, failMessage } = ctx
  ...
  try {
    if (effort !== undefined) {
      await request('config.set', { key: 'reasoning', session_id: sessionId, value: effort })
    }

    if (fast !== undefined) {
      await request('config.set', { key: 'fast', session_id: sessionId, value: fast ? 'fast' : 'normal' })
    }
  } catch (err) {
