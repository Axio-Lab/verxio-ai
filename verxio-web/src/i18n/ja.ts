...
export const ja = defineLocale({
  ...
  notifications: {
    ...
    backendOutOfDateMessage:
      'Verxio バックエンドがこのデスクトップビルドより古く、正常に動作しない場合があります。更新して揃えてください。',
    updateHermes: 'Verxio を更新',
    ...
    errors: {
      ...
      methodNotAllowed:
        'デスクトップバックエンドがそのリクエストを拒否しました (405 Method Not Allowed)。Verxio を再起動してください。',
  ...
  settings: {
    ...
    resetConfirm: 'すべての設定を Verxio のデフォルトに戻しますか？',
    ...
    socials: {
      title: 'ソーシャル',
      intro:
        'セルフホストの Postiz で投稿を予約・公開できます。複数アカウントを接続し、カレンダーを開き、エージェントに下書きと予約を任せられます。',
      loading: 'ソーシャルを読み込み中…',
      failedLoad: 'ソーシャル状態を読み込めませんでした。',
      ready: '準備完了',
      needsKey: 'セットアップが必要',
      disabled: '未有効',
      unavailable: '利用不可',
      notConfigured: 'このデプロイでは Postiz が動いていません。Postiz を起動してから有効化してください。',
      setupTitle: 'このワークスペースの Postiz',
      enabledDescription: 'ソーシャル予約が有効です。チャンネルを接続するかカレンダーを開いて投稿を管理します。',
      disabledDescription: 'ソーシャルを有効にすると、無料の Postiz ワークスペースが用意されます。',
      enable: 'ソーシャルを有効化',
      disable: '無効化',
      disableConfirm:
        'このワークスペースのソーシャルを無効にしますか？接続済みチャンネルは削除するまで Postiz に残ります。',
      enableFailed: 'ソーシャルを有効化できませんでした。',
      disableFailed: 'ソーシャルを無効化できませんでした。',
      enabledTitle: 'ソーシャルを有効化しました',
      enabledMessage: 'このワークスペースは Postiz で投稿を予約できます。',
      disabledTitle: 'ソーシャルを無効化しました',
      disabledMessage: 'エージェントランタイムから Postiz ツールを外しました。',
      openCalendar: 'カレンダーを開く',
      calendarTitle: 'コンテンツカレンダー',
      calendarDescription: '予約投稿を確認・編集します。ここに表示されない場合は新しいウィンドウで開いてください。',
      calendarFailed: 'カレンダーを開けませんでした。',
      openInNewWindow: '新しいウィンドウで開く',
      connectChannel: 'チャンネルを接続',
      connectTitle: 'ソーシャルアカウントを接続',
      connectDescription: 'ネットワークを選びます。複数アカウントを接続でき、毎回 Postiz OAuth が開きます。',
      connectFailed: 'チャンネル接続を開始できませんでした。',
      connectOpenedTitle: 'Postiz で続行',
      connectOpenedMessage: '開いたウィンドウで接続を完了してから戻ってください。',
      channelsTitle: '接続済みチャンネル',
      noChannelsTitle: 'チャンネルがありません',
      noChannels: 'LinkedIn、X、Instagram などを接続できます。複数アカウントを追加可能です。',
      disconnect: '切断',
      disconnectConfirm: 'このチャンネルを Postiz から切断しますか？',
      disconnectFailed: 'チャンネルを切断できませんでした。',
      disconnectedTitle: 'チャンネルを切断しました',
      disconnectedMessage: 'このアカウントをワークスペースのカレンダーから外しました。',
      channelActive: '有効',
      channelDisabled: '無効',
      postsTitle: '今後の投稿',
      noPostsTitle: '予約投稿はありません',
      noPosts: 'カレンダーで下書きまたは予約すると、ここに表示されます。',
      unscheduledPost: '未予約',
      draftPost: '下書き'
    },
    runtime: {
    ...
    appearance: {
      ...
      colorMode: 'カラーモード',
      colorModeDesc: '固定モードを選ぶか、Verxio をシステム設定に合わせます。',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone:
        'Verxio がローカル時刻のコンテキストを必要とするときに使用します。空欄ならシステムのタイムゾーンを使います。',
      agent: {
        imageInputMode: '画像添付をモデルへ送る方法を制御します。',
        maxTurns: 'Verxio が 1 回の実行を停止するまでのツール呼び出しターン上限です。'
      },
      terminal: {
      ...
      fileReadMaxChars: 'Verxio が 1 回のファイル読み取りで取得できる最大文字数です。',
      approvals: {
        mode: '明示的な承認が必要なコマンドを Verxio がどう扱うかを設定します。',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'アプリから Verxio 自身を更新するとき、ローカルのソース変更を保持するか破棄するかを選びます。ターミナル更新では常に確認されます。'
      }
    }),
    about: {
      heading: 'Verxio',
  ...
  commandCenter: {
    ...
    nav: {
      ...
      settings: { title: '設定', detail: 'Verxio デスクトップを設定' },
    ...
    hermesActiveSessions: (version, count) => `Verxio ${version} · アクティブセッション ${count}`,
  ...
  profiles: {
    ...
    deleting: '削除中...',
    createDesc: 'プロファイルは独立した Verxio 環境です：設定、スキル、SOUL.md が別々になります。',
    nameLabel: '名前',
  ...
  cron: {
    ...
    emptyDescNew:
      'Cron 式でプロンプトを実行するスケジュールを設定します。Verxio が実行して、選択した宛先に結果を送信します。',
  ...
  composer: {
    ...
    placeholderStarting: 'Verxio を起動中...',
    placeholderReconnecting: 'Verxio に再接続中…',
    ...
    attachUrlDesc: 'Verxio がページを取得し、このターンのコンテキストとして含めます。',
  ...
  updates: {
    ...
    unsupportedMessage: 'このバージョンの Verxio はアプリ内から自分を更新できません。',
    connectionRetry: '接続を確認してもう一度試してください。',
    latestBody: '最新バージョンを実行しています。',
    allSetTitle: '準備完了',
    availableTitle: '新しい更新が利用可能',
    availableBody: '新しいバージョンの Verxio をインストールする準備ができています。',
    updateNow: '今すぐ更新',
    maybeLater: '後で',
    moreChanges: count => `さらに ${count} 件の変更が含まれています。`,
    manualTitle: 'ターミナルから更新',
    manualBody:
      'Verxio をコマンドラインからインストールしたため、更新もそこで実行されます。これをターミナルに貼り付けてください:',
    manualPickedUp: 'Verxio は次回起動時に新しいバージョンを読み込みます。',
    copy: 'コピー',
    copied: 'コピーしました',
    done: '完了',
    applyingBody: 'Verxio アップデーターが独自のウィンドウで引き継ぎ、完了後に Verxio を再度開きます。',
    applyingClose: 'Verxio は更新を適用するために閉じます。',
  ...
  install: {
    ...
    oneTimeTitle: 'Verxio には一度限りのインストールが必要です',
    ...
    settingUpTitle: 'Verxio を設定中',
    finishingTitle: '仕上げ中',
    failedDesc:
      'インストール手順のいずれかが失敗しました。Windows では、別の Verxio CLI またはデスクトップインスタンスが実行中の場合に発生することがあります。実行中の Verxio インスタンスをすべて停止してから再試行してください。詳細は以下またはデスクトップログで確認できます。',
    activeDesc:
      'これは一回限りのセットアップです。Verxio インストーラーが依存関係をダウンロードしてマシンを設定しています。以降の起動ではこの手順はスキップされます。',
  ...
  onboarding: {
    headerTitle: 'Verxio のセットアップをしましょう',
    headerDesc: 'チャットを始めるにはモデルプロバイダーを接続してください。ほとんどのオプションはワンクリックです。',
    preparingInstall: 'Verxio はインストールを完了中です。初回実行では通常 1 分以内に完了します。',
    starting: 'Verxio を起動中…',
    ...
    recommended: '推奨',
    connected: '接続済み',
    featuredPitch: '1 つのサブスクリプションで 300 以上の最先端モデル — Verxio を実行するための推奨方法',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: 'セルフホスト',
        description:
          'ローカルまたはセルフホストの OpenAI 互換エンドポイント（vLLM、llama.cpp、Ollama など）に Verxio を接続。'
      }
    },
    ...
    flowSubtitles: {
      ...
      device_code: 'ブラウザーで確認ページを開きます — Verxio が自動接続します',
      loopback: 'サインインのためブラウザーを開きます — Verxio が自動接続します',
  ...
  modelPicker: {
    ...
    pro: 'Pro',
    proNeedsSubscription: 'Pro モデルには有料の Verxio サブスクリプションが必要です。',
    free: '無料',
    freeTier: '無料プラン',
  ...
  shell: {
    ...
    statusbar: {
      ...
      gatewayTitle: 'Verxio 推論ゲートウェイのステータス',
      agents: 'エージェント',
  ...
  preview: {
    ...
    largeBody: (label, size) => `${label} は ${size} です。Verxio は最初の 512 KB のみを表示します。`,
    ...
    web: {
      ...
      tryAgain: '再試行',
      restarting: 'Verxio を再起動中...',
      askRestart: 'Verxio にサーバーの再起動を依頼',
      lookingRestart: taskId => `Verxio は再起動するプレビューサーバーを検索中です (${taskId})`,
      restartingTitle: 'プレビューサーバーを再起動中',
      restartingMessage: 'Verxio はバックグラウンドで作業中です。進捗はプレビューコンソールで確認してください。',
      ...
      finishedRestarting: message =>
        `Verxio がプレビューサーバーの再起動を完了しました${message ? `: ${message}` : ''}`,
      ...
      restartFailedMessage: 'Verxio がサーバーを再起動できませんでした。',
      stillWorking:
        'Verxio はまだ作業中ですが、再起動の結果がまだ届いていません。サーバーコマンドがフォアグラウンドで実行されている可能性があります。',
  ...
  prompts: {
    ...
    sudoTitle: '管理者パスワード',
    sudoDesc:
      'Verxio は特権コマンドを実行するために sudo パスワードが必要です。ローカルエージェントにのみ送信されます。',
  ...
  desktop: {
    ...
    noModelSelected: 'モデルが選択されていません。送信前にモデルを選択してください。',
    ...
    restartToUseSaveImage: '画像を保存するには Verxio を再起動してください。',
    restartToSaveImages: '画像を保存するには Verxio を再起動してください',
