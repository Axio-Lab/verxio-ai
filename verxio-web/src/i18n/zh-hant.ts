...
export const zhHant = defineLocale({
  ...
  boot: {
    ...
    steps: {
      ...
      loadingSettings: '正在載入 Verxio 設定',
  ...
  notifications: {
    ...
    backendOutOfDateMessage: '您的 Verxio 後端早於目前的桌面版本，可能無法正常運作。請更新以保持一致。',
    ...
    errors: {
      ...
      methodNotAllowed: '桌面後端拒絕了該請求 (405 Method Not Allowed)。請嘗試重新啟動 Verxio。',
  ...
  settings: {
    ...
    resetConfirm: '要將所有設定恢復為 Verxio 預設值嗎？',
    exportFailed: '匯出失敗',
    resetFailed: '重設失敗',
    nav: {
      providers: '提供方',
    ...
    socials: {
      title: '社群',
      intro: '透過自架 Postiz 排程並發布社群貼文。可連接多個帳號、開啟日曆，並讓代理起草與排程。',
      loading: '正在載入社群…',
      failedLoad: '無法載入社群狀態。',
      ready: '就緒',
      needsKey: '需要設定',
      disabled: '未啟用',
      unavailable: '無法使用',
      notConfigured: '此部署尚未執行 Postiz。請先啟動 Postiz，再啟用社群。',
      setupTitle: '此工作區的 Postiz',
      enabledDescription: '社群排程已啟用。連接頻道或開啟日曆管理貼文。',
      disabledDescription: '啟用社群以為此工作區開通免費 Postiz 排程。',
      enable: '啟用社群',
      disable: '停用',
      disableConfirm: '停用此工作區的社群？已連接頻道仍保留在 Postiz，直到你移除為止。',
      enableFailed: '無法啟用社群。',
      disableFailed: '無法停用社群。',
      enabledTitle: '社群已啟用',
      enabledMessage: '此工作區可透過 Postiz 排程發文。',
      disabledTitle: '社群已停用',
      disabledMessage: '已從此代理執行環境移除 Postiz 工具。',
      openCalendar: '開啟日曆',
      calendarTitle: '內容日曆',
      calendarDescription: '檢視並編輯已排程貼文。若此處無法載入，請用新視窗開啟。',
      calendarFailed: '無法開啟日曆。',
      openInNewWindow: '在新視窗開啟',
      connectChannel: '連接頻道',
      connectTitle: '連接社群帳號',
      connectDescription: '選擇網路。可連接多個帳號；每次都會開啟 Postiz OAuth。',
      connectFailed: '無法開始連接頻道。',
      connectOpenedTitle: '請在 Postiz 繼續',
      connectOpenedMessage: '在開啟的視窗完成帳號連接後再回到這裡。',
      channelsTitle: '已連接頻道',
      noChannelsTitle: '尚無頻道',
      noChannels: '連接 LinkedIn、X、Instagram 等。可新增多個帳號。',
      disconnect: '中斷連接',
      disconnectConfirm: '從此 Postiz 中斷此頻道？',
      disconnectFailed: '無法中斷頻道。',
      disconnectedTitle: '頻道已中斷',
      disconnectedMessage: '該帳號已從此工作區日曆移除。',
      channelActive: '使用中',
      channelDisabled: '已停用',
      postsTitle: '即將發布',
      noPostsTitle: '尚無排程貼文',
      noPosts: '在日曆中起草或排程貼文後，會顯示在這裡。',
      unscheduledPost: '未排程',
      draftPost: '草稿'
    },
    runtime: {
    ...
    appearance: {
      ...
      colorMode: '色彩模式',
      colorModeDesc: '選擇固定模式，或讓 Verxio 跟隨系統設定。',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone: 'Verxio 需要本機時間上下文時使用。留空則使用系統時區。',
      agent: {
        imageInputMode: '控制圖片附件如何傳送給模型。',
        maxTurns: 'Verxio 停止一次執行前的工具呼叫輪次上限。'
      },
      terminal: {
      ...
      fileReadMaxChars: 'Verxio 單次檔案讀取可讀取的最大字元數。',
      approvals: {
        mode: 'Verxio 如何處理需要明確批准的指令。',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'Verxio 從應用程式內更新自身時，保留本機原始碼變更（stash）或丟棄（discard）。終端機更新一律會詢問。'
      }
    }),
    about: {
      heading: 'Verxio',
      description: 'Verxio 是一款自我改進的 AI 智慧體，可在聊天、訊息和已連結應用中為團隊完成工作。'
    },
    config: {
      none: '無',
      noneParen: '(無)',
    ...
    gateway: {
      ...
      envOverride: '環境變數覆寫',
      intro:
        'Verxio 預設會啟動自己的本機閘道。如果您希望此應用程式控制另一台機器或可信代理後面已執行的 Verxio 後端，請使用遠端閘道。在下方按設定檔指定各自的遠端主機。',
      appliesTo: '套用至',
      allProfiles: '全部設定檔',
      ...
      localTitle: '本機閘道',
      localDesc: '在 localhost 啟動私有 Verxio 後端。這是預設方式，可離線使用。',
      remoteTitle: '遠端閘道',
      remoteDesc:
      ...
      savedTitle: '閘道設定已儲存',
      restartingMessage: 'Verxio 將使用已儲存的設定重新連線。',
      savedMessage: '已儲存，下次重新啟動後生效。',
      connectedTo: (baseUrl, version) => `已連線至 ${baseUrl}${version ? ` · Verxio ${version}` : ''}`,
  ...
  skills: {
    ...
    editor: {
      ...
      deleteDesc: name => `將從此設定檔永久刪除「${name}」技能，且無法復原。`,
      deleteConfirm: '刪除技能',
      deleting: '刪除中…',
      deleted: '技能已刪除',
      importPackage: '匯入套件',
  ...
  commandCenter: {
    ...
    nav: {
      ...
      settings: { title: '設定', detail: '設定 Verxio 桌面端' },
    ...
    hermesActiveSessions: (version, count) => `Verxio ${version} · 活躍工作階段 ${count}`,
  ...
  profiles: {
    ...
    deleting: '刪除中…',
    createDesc: '設定檔是獨立的 Verxio 環境：各自擁有獨立的設定、技能和 SOUL.md。',
    nameLabel: '名稱',
  ...
  cron: {
    ...
    emptyDescNew: '按 cron 表達式排程一個提示詞。Verxio 會執行它，並將結果傳送至您選擇的目的地。',
  ...
  artifacts: {
    ...
    deleted: '成品已刪除',
    selectAll: '全選可見成品',
    selectHint: '選擇要批次刪除的項目',
    selectItem: label => `選擇 ${label}`,
    selectedCount: count => `已選 ${count} 項`,
    deleteSelected: count => `刪除 ${count} 項`,
    batchDeleteTitle: count => `刪除 ${count} 個成品？`,
    batchDeleteDescription: count => `將永久刪除所選的 ${count} 個成品。此操作無法復原。`,
    batchDeleted: count => `已刪除 ${count} 個成品`,
    batchDeleteFailed: '部分成品刪除失敗',
    batchDeletePartial: (deleted, failed) => `已刪除 ${deleted}，失敗 ${failed}`,
    previewDescription: '預覽產生的成品內容',
    downloadAction: '下載',
  ...
  composer: {
    ...
    placeholderStarting: '正在啟動 Verxio...',
    placeholderReconnecting: '正在重新連線至 Verxio…',
    ...
    attachUrlDesc: 'Verxio 將擷取該頁面並作為此回合的脈絡。',
  ...
  updates: {
    ...
    unsupportedMessage: '此版本的 Verxio 無法在應用程式內自行更新。',
    connectionRetry: '請檢查網路連線後重試。',
    latestBody: '您正在執行最新版本。',
    allSetTitle: '已是最新版本',
    availableTitle: '有可用更新',
    availableBody: '新版 Verxio 已可安裝。',
    updateNow: '立即更新',
    maybeLater: '稍後再說',
    moreChanges: count => `另有 ${count} 項變更。`,
    manualTitle: '從終端機更新',
    manualBody: '您是從命令列安裝的 Verxio，因此更新也需要在那裡執行。請將此指令貼到終端機：',
    manualPickedUp: '下次啟動 Verxio 時會使用新版本。',
    copy: '複製',
    copied: '已複製',
    done: '完成',
    applyingBody: 'Verxio 更新程式會在自己的視窗中接管，並在完成後重新開啟 Verxio。',
    applyingClose: 'Verxio 將關閉以套用更新。',
    errorTitle: '更新未完成',
  ...
  install: {
    ...
    failedTitle: '安裝失敗',
    settingUpTitle: '正在設定 Verxio',
    finishingTitle: '正在收尾',
    failedDesc:
      '某個安裝步驟失敗。在 Windows 上，如果另一個 Verxio CLI 或桌面執行個體正在執行，可能會出現這種情況。請停止正在執行的 Verxio 執行個體後重試。可查看下方的詳細資訊或 desktop 記錄中的完整記錄。',
    activeDesc: '這是一次性設定。Verxio 安裝程式正在下載相依套件並設定您的電腦。之後啟動會略過此步驟。',
  ...
  onboarding: {
    headerTitle: '開始設定 Verxio',
    headerDesc: '連線模型提供方即可開始聊天。大多數選項只需一次點擊。',
    preparingInstall: 'Verxio 正在完成安裝。首次執行通常不到一分鐘。',
    starting: '正在啟動 Verxio…',
    ...
    recommended: '建議',
    connected: '已連線',
    featuredPitch: '一個訂閱，300+ 前沿模型 — 執行 Verxio 的建議方式',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: '自託管',
        description: '將 Verxio 指向本機或自託管的 OpenAI 相容端點（vLLM、llama.cpp、Ollama 等）。'
      }
    },
    backToSignIn: '返回登入',
    getKey: '取得金鑰',
    ...
    flowSubtitles: {
      ...
      device_code: '在瀏覽器中開啟驗證頁面 — Verxio 會自動連線',
      loopback: '開啟瀏覽器登入 — Verxio 會自動連線',
    ...
    authorizeThere: '請在那裡授權 Verxio。',
    copyAuthCode: '複製授權碼並貼到下方。',
    copyCallbackUrl: '登入後，從瀏覽器複製完整的回呼 URL 並貼到下方。',
    pasteAuthCode: '貼上授權碼',
    pasteCallbackUrl: '貼上回呼 URL（http://127.0.0.1:56121/callback?...）',
    reopenAuthPage: '重新開啟授權頁面',
    autoBrowser: provider => `已在瀏覽器中開啟 ${provider}。請在那裡授權 Verxio，連線會自動完成，無需複製或貼上。`,
  ...
  modelPicker: {
    ...
    pro: 'Pro',
    proNeedsSubscription: 'Pro 模型需要付費 Verxio 訂閱。',
    free: '免費',
    freeTier: '免費層',
  ...
  preview: {
    ...
    largeTitle: '此檔案較大',
    largeBody: (label, size) => `${label} 大小為 ${size}。Verxio 只會顯示前 512 KB。`,
    previewAnyway: '仍然預覽',
    ...
    web: {
      ...
      failedToLoad: '預覽載入失敗',
      tryAgain: '重試',
      restarting: 'Verxio 正在重新啟動...',
      askRestart: '請 Verxio 重新啟動伺服器',
      lookingRestart: taskId => `Verxio 正在尋找要重新啟動的預覽伺服器 (${taskId})`,
      restartingTitle: '正在重新啟動預覽伺服器',
      restartingMessage: 'Verxio 正在背景執行。可在預覽主控台查看進度。',
      ...
      finishedRestarting: message => `Verxio 已完成預覽伺服器重新啟動${message ? `：${message}` : ''}`,
      failedRestarting: message => `伺服器重新啟動失敗：${message}`,
      unknownError: '未知錯誤',
      restartedTitle: '預覽伺服器已重新啟動',
      reloadingNow: '正在重新載入預覽。',
      restartFailedTitle: '預覽重新啟動失敗',
      restartFailedMessage: 'Verxio 無法重新啟動伺服器。',
      stillWorking: 'Verxio 仍在執行，但尚未收到重新啟動結果。伺服器指令可能正在前台執行。',
  ...
  prompts: {
    ...
    sudoTitle: '管理員密碼',
    sudoDesc: 'Verxio 需要您的 sudo 密碼來執行特權指令。它只會傳送給您的本機代理。',
  ...
  desktop: {
    ...
    noModelSelected: '尚未選擇模型。請先選擇模型再傳送。',
    ...
    restartToUseSaveImage: '重新啟動 Verxio 後可使用儲存圖片。',
    restartToSaveImages: '重新啟動 Verxio 以儲存圖片',
