...
const CY = 280;
const RADIUS = 240;
const RADIUS_PERCENT = 42.8;

/** Round trig results so SSR HTML matches the client (avoids float hydration mismatches). */
function round(n: number, digits = 4): number {
  const factor = 10 ** digits;
  return Math.round(n * factor) / factor;
}

const ORBIT_LAYOUT = ORBIT_APPS.map((_, i) => {
  const angleRad = (i / ORBIT_APPS.length) * 2 * Math.PI - Math.PI / 2;
  const angleDeg = (i / ORBIT_APPS.length) * 360 - 90;

  return {
    x: round(CX + Math.cos(angleRad) * RADIUS),
    y: round(CY + Math.sin(angleRad) * RADIUS),
    left: round(50 + RADIUS_PERCENT * Math.cos((angleDeg * Math.PI) / 180)),
    top: round(50 + RADIUS_PERCENT * Math.sin((angleDeg * Math.PI) / 180)),
  };
});
